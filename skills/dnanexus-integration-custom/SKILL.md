---
name: dnanexus-integration-custom
description: Custom fork of the DNAnexus integration skill for reproducible genomics workloads with dx CLI, dxpy, apps/applets, native workflows, dxCompiler, and Nextflow. Use for local fork-specific DNAnexus automation, nf-core/bacass testing, Nextflow output publishing, resume/cache handling, and project monitoring.
license: MIT
...
# DNAnexus Integration Custom

## Purpose

Use this custom fork skill to build, run, and operate DNAnexus workloads
without guessing at platform semantics. It covers:
...
The documented baseline was verified on **2026-08-12** against
`dxpy==0.412.0`, dxCompiler 2.17.0, the 2026 DNAnexus documentation, and
DNAnexus Academy Nextflow guidance. Consult `references/sources.md` and
current release notes when behavior may have changed.
...
- `dx-toolkit` v0.412.0 fixes a Nextflow executor issue where workflows could
  become stuck waiting for temporary upload failures. If a Nextflow app was
  built with an older toolkit and shows that symptom, upgrade `dx` and rebuild
  the app or applet before rerunning.
- For DNAnexus Nextflow applets, prefer generated applet inputs for pipeline
  parameters exposed by `nextflow_schema.json`. For nf-core-style `outdir`,
  pass a relative head-node path such as `-ioutdir=./results` and put the
  Platform folder in `--destination`. Do not pass `dx://project-...:/...` as
  `outdir`; Academy and platform docs treat `outdir` as local to the Nextflow
  head job, and absolute or DNAnexus URI publish paths can skip or fail output
  publication.
- A `PermissionDenied` error while copying published Nextflow outputs is not a
  temporary upload stall. Check the applet access, destination project/folder
  permission, and output path construction; rerun with a relative `outdir` and
