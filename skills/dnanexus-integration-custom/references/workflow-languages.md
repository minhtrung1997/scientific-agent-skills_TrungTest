...
Use `dx-toolkit` v0.412.0 or newer when building Nextflow apps/applets. The
2026-08-04 DNAnexus release notes say v0.412.0 lets Nextflow workflows recover
from temporary upload failures instead of remaining stuck waiting for an upload
to finish; apps built with older toolkits should be rebuilt before rerunning
when that symptom appears.
...
When the generated applet exposes pipeline parameters from
`nextflow_schema.json`, pass those values through the applet inputs instead of
duplicating them in `nextflow_pipeline_params`. DNAnexus recommends the app or
applet interface because it validates input classes and object existence before
creating the job.

For nf-core-style pipelines that use `params.outdir` in `publishDir`, set
`outdir` to a relative path on the Nextflow head job and use `--destination`
for the DNAnexus project folder:
...
This publishes under `project-xxxx:/runs/run-003/results/...`. Do not set
`outdir` to `/results` or `dx://project-xxxx:/results`; Academy guidance says
`outdir` is local to the head node, and the platform docs describe final
publication as copying the local `publishDir` tree into the job destination.
Use `nextflow_pipeline_params` for extra CLI-style parameters that are not
exposed as applet inputs. If those parameters refer to DNAnexus files or
folders, use DNAnexus URI or qualified-ID syntax according to the generated
input type.
...
On failures, the Nextflow log is written to the job `--destination`, but
pipeline result files under `params.outdir` are copied to the project only
after successful completion unless the pipeline explicitly uses an error
strategy such as `ignore`. If a job is manually terminated or exceeds its cost
limit, cache may not be preserved even when `preserve_cache=true`; reruns then
cannot resume from that terminated job.

A publish-time `PermissionDenied` error while copying from
`.nextflow_cache_db/<session_id>/work/...` to the destination is a project
permission or output-path problem, not the temporary upload failure fixed in
dx-toolkit v0.412.0. Check the generated applet's project access, the launch
user's write access to the destination, and whether `outdir` was passed as a
relative local path.
