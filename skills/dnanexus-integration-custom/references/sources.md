...
|---|---|
| DNAnexus Platform docs | 2026 documentation and release notes through 2026-08-04 |
| DNAnexus Academy docs | Nextflow applet and nf-core workflow guidance checked 2026-08-12 |
| dx-toolkit / dxpy | 0.412.0, current command docs client v0.412.0 |
...
| dxFUSE | 1.6.1 |
| Nextaur | 1.13.0 |
| Nextflow engines exposed by dx-toolkit 0.412.0 | 25.10 and 24.10 |
...
- 2026-08-04: dx-toolkit 412.0 lets Nextflow workflows recover from temporary
  upload errors instead of remaining stuck waiting for upload; upgrade `dx` and
  rebuild affected Nextflow apps
...
- [DNAnexus Academy: Building Nextflow Applets](https://academy.dnanexus.com/buildingworkflows/nf/buildingnextflowapplets)
- [DNAnexus Academy: Importing Nf-Core](https://academy.dnanexus.com/buildingworkflows/nf/importingandrunningnfcore)
- [DNAnexus Academy: Job Failures](https://academy.dnanexus.com/buildingworkflows/nf/jobfailures)
...
| Treating every `wait_on_done()` exception as remote failure | In dxpy 0.412.0, `DXJobFailureError` also covers termination and local wait timeout; re-describe state |
...
| Passing nf-core `outdir` as `dx://project-...:/results` | Pass a relative head-node path such as `-ioutdir=./results` and use `--destination` for the DNAnexus project folder |
