...
This reference was verified against `dxpy==0.412.0` on 2026-08-12. PyPI
declares Python 3.8 or newer. This repository recommends Python 3.11+.
...
```bash
uv run --with "dxpy==0.412.0" "script.py"
```
...
`DXFile.open_file()` does not exist in dxpy 0.412.0. Use
`dxpy.open_dxfile()`.
...
Useful run arguments in dxpy 0.412.0 include:
...
matches the caller's needs. In dxpy 0.412.0, `DXJobFailureError` represents
...
top-level dxpy exception classes in 0.412.0.
